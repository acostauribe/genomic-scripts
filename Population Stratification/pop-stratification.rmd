...
knitr::opts_chunk$set(root.dir = "~/Documents/Kosik Lab/ReD-Lat/admixture/",
                      dev = "png",
                      dpi = 300,
                      echo = FALSE,
                      cache = TRUE)
setwd("~/Documents/Kosik Lab/ReD-Lat/admixture/")
...
prefix='redlat-array.qc'
Sys.setenv(prefix=prefix)

# Gibe the additional information on your individuals as a text file ej. 
...
```{r load-sample-data}
sample_data = read.delim(sample_data, header = TRUE )
eigenvec$POPULATION = sample_data$POPULATION[match(eigenvec$IID, sample_data$IID)]
```
...
ggplot(eigenvec, aes(x=PC1, y=PC2, color=POPULATION)) + 
        ...
        labs(title="Principal Component Analysis PC1 vs PC2", 
            caption="Array Batch 4 July 2023", fill = "Country")
...
ggplot(eigenvec, aes(x=PC2, y=PC3, color=POPULATION)) + 
        ...
        labs(title="Principal Component Analysis PC2 vs PC3", 
            caption="Array Batch 4 July 2023")
...
Unsupervised mode
```{bash run-admixture}
# Choose number of iterations you want to do
declare -i iterations=20
declare -i k_model=10

for i in $(seq 1 $iterations)
do
        echo "Starting" Iteration_${i}
  	mkdir Iteration_${i}
        for k in $(seq 1 $k_model) # choose number of ancestral populations (K) you want to model
        do
...
```{bash cv-error, eval=FALSE, include=FALSE}
declare -i iterations=20
declare -i k_model=10
...
```{bash edit-cv-error}
sed -i 's/CV error (K=//g' $prefix.CVerror.txt > o
...
# Plot with Base R
plot(admixture_data$K, 
     admixture_data$CV.error, 
     pch = 16, 
     col = "blue", 
...
# Add mean lines
lines(mean_cv$K, mean_cv$CV.error, col = "red", lty = "dashed")


# You can do the same plot with ggplot2
...
for i in $(seq 1 $iterations)
do
        for k in $(seq 2 $k_model) # choose number of ancestral populations (K) you want to model
        do
...
**Make the IND2POP file**

Load your fam and use the sample_data to get the population that each individual belongs to
```{r load-famdata}
fam = read.delim(paste0(prefix,".unr.maf.ld.fam"), sep=' ', header=FALSE)
fam$V7 = sample_data$POPULATION[match(fam$V2, sample_data$IID)]

# Get the populations in the same order
ind2pop = fam$V7 
...
**Make the POP_NAMES file**
Get all the unique population values and sort them alphabetically

```{r pop-names}
pop_names = sort(unique(ind2pop))
write.table(pop_names,
            "pop_names.txt", 
            col.names = FALSE, 
            row.names = FALSE, 
            quote = FALSE)
...
$pong --filemap filemap.txt --ind2pop ind2pop.txt --pop_names pop_names.txt  --verbose
```
